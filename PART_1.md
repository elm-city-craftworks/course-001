...
* Run the following bash commands in the `data` folder and copy the 
...
* Get the test harness in `ls_tests.rb` to pass its first test.
...
* Now replace the `ruby-ls` script with a Ruby-based implementation 
that passes the first test.

* Work your way through implementing some or all of the other use cases
listed in step 1. Start by adding an acceptance test, then implement
the correct behavior in the `ruby-ls` script.

* If you didn't already check for exit codes for successful and unsuccessful
